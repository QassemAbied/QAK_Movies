...
class MovieDetailsBloc extends Bloc<MovieDetailEvent, MovieDetailsState> {
  ...
  MovieDetailsBloc(
    ...
    on<DetailsEvent>((event, emit) async {
      ...
      response.fold((l) {
        emit(state.copyWith(
            enumRequestdetails: EnumRequest.error,
            messagedetails: l.toString()));
      }, (r) {
        emit(state.copyWith(
            detailsData: r, enumRequestdetails: EnumRequest.loded));
      });
    });
    ...
    on<ImageForMovieEvent>((event, emit) async {
      ...
      response.fold((l) {
        emit(state.copyWith(
            enumRequestimageMovie: EnumRequest.error,
            messageimageMovie: l.toString()));
      }, (r) {
        emit(state.copyWith(
            imageMovieData: r, enumRequestimageMovie: EnumRequest.loded));
      });
    });
    ...
    on<CastMovieEvent>((event, emit) async {
      ...
      response.fold((l) {
        emit(state.copyWith(
            enumRequestcastMovie: EnumRequest.error,
            messagecastMovie: l.toString()));
      }, (r) {
        emit(state.copyWith(
            castMovieData: r, enumRequestcastMovie: EnumRequest.loded));
      });
    });
    ...
    on<SimilarMovieEvent>((event, emit) async {
      ...
      response.fold((l) {
        emit(state.copyWith(
            enumRequestsimilarMovie: EnumRequest.error,
            messagesimilarMovie: l.toString()));
      }, (r) {
        emit(state.copyWith(
            similarMovieData: r, enumRequestsimilarMovie: EnumRequest.loded));
      });
    });
    ...
    on<CheckAccountStatesEvent>((event, emit) async {
      ...
      response.fold((l) {
        emit(state.copyWith(
            enumRequestcheckaccount: EnumRequest.error,
            messagecheckaccount: l.toString()));
      }, (r) {
        emit(state.copyWith(
            checkaccount: r, enumRequestcheckaccount: EnumRequest.loded));
      });
    });
    ...
    on<VideoMovieEvent>((event, emit) async {
      ...
      response.fold((l) {
        emit(state.copyWith(
            enumRequestvideo: EnumRequest.error, messagevideo: l.toString()));
      }, (r) {
