...
class AuthenticationBloc
  ...
  AuthenticationBloc(this.requestTokenUseCase, this.loginValidateUseCase,
    ...
    on<RequestTokenEvent>((event, emit) async {
      ...
      result.fold((l) {
        emit(state.copyWith(
            enumRequesttoken: EnumRequest.error, message: l.message));
      }, (r) {
        emit(state.copyWith(
            requestToken: r, enumRequesttoken: EnumRequest.loded));
      });
    });
    ...
    on<LoginValidateEvent>((event, emit) async {
      ...
      result.fold((l) {
        emit(state.copyWith(
            enumRequestlogin: EnumRequest.error, messagelogin: l.message));
      }, (r) {
    ...
    on<CreateSessionEvent>((event, emit) async {
      ...
      result.fold((l) {
        emit(state.copyWith(
            enumRequestsession: EnumRequest.error, messagesession: l.message));
      }, (r) {
